...
const axios_1 = __importDefault(require("axios")); // For fetching remote resources
const mime_1 = __importDefault(require("mime")); // For determining MIME types of local files
...
class TransformStream extends node_stream_1.default.Transform {
     ...
     */
    async readAndConvertFont(fontSrc) {
        // Handle remote fonts (e.g., from Google Fonts CDN)
        if (fontSrc.startsWith('http://') || fontSrc.startsWith('https://')) {
            try {
                // Fetch as binary data (arraybuffer)
                let response = await axios_1.default.get(fontSrc, { responseType: 'arraybuffer' });
                if (response.status === 200) {
            ...
            try {
                // Determine MIME type from file extension
                const mimeType = mime_1.default.getType(fontSrc);
