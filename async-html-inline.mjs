...
class TransformStream extends stream.Transform {
  ...
  async fetchResource(src) {
    if (src.startsWith('http://') || src.startsWith('https://')) {
      ...
      try {
        return await readFileAsync(resolvedPath, 'utf8');
      } catch (error) {
  ...
  async readAndConvertImage(imgSrc) {
    if (imgSrc.startsWith('http://') || imgSrc.startsWith('https://')) {
      ...
      const resolvedPath = path.resolve(__dirname, imgSrc);
      try {
        const mimeType = mime.getType(resolvedPath);
        if (! mimeType.startsWith('image/')) {
          console.error('This is not an image file: ' + resolvedPath);
          return '';
        }
        const imgData = await readFileAsync(resolvedPath);
